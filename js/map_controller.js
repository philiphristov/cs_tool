...
class MapController {
	...
	addCircleMarker(position, label, color) {
		...
		var marker = new this.pixioverlay.pixiMarker({
			...
			clickListener: function() {
				appManager.map_controller.closeAllInfoWindows();
				appManager.data_manager.checkDataBeforeListModal(marker);
			}
		});
	...
	change_marker(marker, number, color) {
		...
		var num_entries = appManager.data_manager.check_for_entries(marker.location_id);
		var cur_user_entries = appManager.data_manager.check_for_current_user_entries(marker.location_id);
		...
		if (num_entries <= 0) {
			...
			var new_marker = appManager.map_controller.addCircleMarker(position, label, color);
